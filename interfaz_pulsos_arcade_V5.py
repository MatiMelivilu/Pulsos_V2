...
class App(tk.Tk):
    ...
    def POS_listener(self):
        ...
        while True:
            ...
            try:            
                        ...
                        if "0911" in codigos:
                            ...
                            if self.esperar_ack():
                                ...
                                if self.verificar_conexion_pulsos():
                                    mlog = f"[{current_time}] Verificacion de pulsos exitosa\n"
                                    self.log(mlog)
                                    self.log_file.write(mlog)
                                    self.log_file.flush()
                ...
                self.log(f"Desconectado. Error: {e}")
                self.log_file.write(f"Desconectado. Error: {e}")
                self.log_file.flush()
    ...
    def interpretar_respuesta_0210(self, linea, n):
        ...
        if codigo_respuesta == "00":
            ...
            self.log_file.flush()
            self.enviar_ack()
            self.log("cerrando conexion de pulsos")
            self.log_file.write("cerrando conexion de pulsos")
            self.log_file.flush()            
            self.serPulsos.close()
