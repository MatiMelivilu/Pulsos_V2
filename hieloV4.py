...
#Configurion de los puertos GPIO        
    ...
    def pulse_test(self):
         ...
         send_button1 = tk.Button(self.test_frame, text="test venta", font=font_large, width=pos_width, height=pos_height, command=self.test_pulso)
    ...
    def freePassRead(self):
        global ruta_freePass
        free = None   
        while True:
            try:
                if free is None or not free.is_open:
                    free = serial.Serial(ruta_freePass, baudrate=9600, timeout=1)
                    current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    mlog = f"[{current_time}] FreePass conectado.\n"
                    self.log(mlog)
                    self.log_file.write(mlog)
                    self.log_file.flush()
                
                if free.in_waiting > 0:
                    data = free.readline().decode(errors='ignore').strip
                    if data:
                        current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        mlog = f"[{current_time}] FreePass activado.\n"
                        self.log(mlog)
                        self.log_file.write(mlog)
                        self.log_file.flush()
                        self.test_pulso()
                        free.reset_input_buffer()
            
            except serial.SerialException:
                print('Error: no se accede a USB freepass, intentando reconectar...')
                if free:
                    free.close()
                    free = None
                time.sleep(2)
    ...
    def verificar_conexion_pulsos(self):
        global ruta_pulsos
        try:
            # Si ya estaba abierto, cerrarlo limpiamente antes de intentar
            if hasattr(self, 'serPulsos') and self.serPulsos:
                try:
                    if self.serPulsos.is_open:
                        self.serPulsos.close()
                except Exception:
                    pass  # Si el cierre falla, ignoramos (el SO lo liberar)

            # Intentar abrir el nuevo puerto
            self.serPulsos = serial.Serial(ruta_pulsos, 9600, timeout=1)
            return True

        except (serial.SerialException, FileNotFoundError, OSError) as e:
            # Log de error limpio
            current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            mlog = f"[{current_time}] ? No se pudo abrir el puerto de pulsos: {e}\n"
            self.log(mlog)
            self.log_file.write(mlog)
            self.log_file.flush()

            # Limpiar la referencia para evitar mal uso posterior
            self.serPulsos = None
            return False
                    ...
                    if self.pos_serial.in_waiting > 0:
                        ...
                        if "0911" in codigos:
                            ...
                            if self.esperar_ack():
                                ...
                                if self.verificar_conexion_pulsos():
                                    self.pos_serial.write(mensaje_venta)
                                else:
                                    self.log("[ERROR] Venta cancelada: no se pudo abrir el puerto de pulsos.")
                            else:
